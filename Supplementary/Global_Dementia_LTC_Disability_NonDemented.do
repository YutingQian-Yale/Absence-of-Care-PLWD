...
************************************************************************
    ...
    *Aim of This Do-file: Disabiltiy Trend (Non-Dementia Sample)
...
*-----------------------------------------------------------------------
*     Overall Trend in Disability (eFig.6: Non-Dementia Sample)
*----------------------------------------------------------------------- 
forvalues i = 1/4{
...
*-----------------------------------------------------------------------
* Save AAPC for Disability - Extent (Never-Demented with any ADLs/IADLs)
...
export excel using "$table/supplementary/efig6.xlsx", replace firstrow(variables) keepcellfmt sheet("efig6_AAC")


*-----------------------------------------------------------------------
* Save for Disability - Extent  (Never-Demented with any ADLs/IADLs)
...
save "$temp/supplementary/efig6_estimates", replace
...
export excel using "$table/supplementary/efig6.xlsx", sheetreplace firstrow(varlabels) keepcellfmt sheet("efig6_ADLIADL")

*-----------------------------------------------------------------------
* Plot eFig.6 

use "$temp/supplementary/efig6_estimates", clear
    ...
    tw line `var'_m year_v2 if data=="HRS",lc("5 113 176") lp(solid) || ///
       ...
       , legend(order(5 "HRS" 6 "ELSA" 7 "SHARE" 8 "CHARLS") ///
        ...
        note("{bf:AAC}" ///
             ...
             ring(0) pos(10) linegap(2) size(*0.7) j(center) box lcolor(white) margin(r=1 t=1 l=1 b=1) ///
             bmargin(l+1 t+1)) ///
...
grc1leg2 ///
   "$temp/supplementary/radliadl" "$temp/supplementary/radltotal" "$temp/supplementary/riadlza" ///
   ,col(3) graphr(margin(zero)) xsize(*1.3) ysize(*0.8) iscale(*1.55) imargin(l=1.5 r=1.5 t=1 b=0) ///
   lsubtsize(*1.2) lmsize(*1.8) labsize(*1.2) 
   
gr export "$figure/supplementary/efig6.png", replace width(5000)   
gr export "$figure/supplementary/efig6.pdf", replace as(pdf) 
