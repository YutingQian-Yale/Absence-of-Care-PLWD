/***********************************************************************
                Disability, Dementia and LTC (Supplementary)
************************************************************************

    *Title: Absence of Care Project
    *Aim of This Do-file: Trend Analysis (Non-Dementia Sample)
...
*-----------------------------------------------------------------------
*        Overall Trend in Absence of Care (eFig7: Never Demented) 
*-----------------------------------------------------------------------
...
forvalues i = 1/4{
    ...
    keep if ragey>=50 & ever_demented==0 & radliadl_01==1
    ...
    * Generate numerical id
        ...
        * Obtain the terminal-year weight for longitudinal data 
        ...
        if `=num_nonmis_year'<=1 { // <=1 wave of observation (i.e.,China; e.g., rracaany) GEE no longer applied
            ...
            if "`var'"== "rrafaany_rev" | "`var'"== "rrifaany_rev"{ 
                ...
                logit `var' year ragey c.radltotal c.riadlza `pweight', vce(robust) nocons
            }
            else{
                logit `var' year ragey ragender c.radltotal c.riadlza `pweight', vce(robust) nocons
            }
            ...
            * GEE        
            xtset newid year   
            xtgee `var' i.year ragey ragender c.radltotal c.riadlza [pw=weight_last] ///
            ...
            * AAPC
            xtgee `var' year ragey ragender c.radltotal c.riadlza [pw=weight_last] ///
                  , family(binomial) link(logit) corr(exchangeable) vce(robust) 
    ...
    foreach var in $varlist0{
        ...
        gen data = "`cty'"
        order data year 
        save "$temp/supplementary/margins_`cty'_`var'", replace 
    }
    clear
    foreach var in $varlist0{
        local use_merge = cond("`var'"=="rrcany_rev", "use", "merge 1:1 year using")
        `use_merge' "$temp/supplementary/margins_`cty'_`var'"
        gen `var'_N = `=`var'_`cty'_N'
        cap drop _merge
    }
    save "$temp/supplementary/margins_`cty'", replace      
    ...
    save "$temp/supplementary/aapc_`cty'_data", replace 
}
...
use "$temp/supplementary/aapc_HRS_data", clear 
append using "$temp/supplementary/aapc_ELSA_data"
append using "$temp/supplementary/aapc_SHARE_data"
append using "$temp/supplementary/aapc_CHARLS_data"
...
export excel using "$table/supplementary/efig7.xlsx", replace firstrow(variables) keepcellfmt sheet("efig7_AAPC")

*-----------------------------------------------------------------------
* Save Point Estimates (95% CI) by Year for Absence of Care

use "$temp/supplementary/margins_HRS", clear 
append using "$temp/supplementary/margins_ELSA"
append using "$temp/supplementary/margins_SHARE"
append using "$temp/supplementary/margins_CHARLS"
...
save "$temp/supplementary/efig7_estimates", replace
...
export excel using "$table/supplementary/efig7.xlsx", sheetreplace firstrow(variables) keepcellfmt sheet("efig7_AbCare")


*-----------------------------------------------------------------------
* Plot eFig.7 

use "$temp/supplementary/efig7_estimates", clear
...
foreach var in $varlist0{
    ...
    tw line `var'_m year_v2 if data=="HRS",lc("5 113 176") lp(solid) || ///
       ...
       , legend(order(5 "HRS" 6 "ELSA" 7 "SHARE" 8 "CHARLS") ///
        ...
        scheme(modern) /// 
        saving("$temp/supplementary/`var'", replace)            
    local i = `i' + 1         
}


grc1leg2 ///
   "$temp/supplementary/rrcany_rev" "$temp/supplementary/rrcaany_rev" "$temp/supplementary/rrfaany_rev" ///
   "$temp/supplementary/rracany_rev" "$temp/supplementary/rracaany_rev" "$temp/supplementary/rrafaany_rev" ///
   "$temp/supplementary/rricany_rev" "$temp/supplementary/rricaany_rev" "$temp/supplementary/rrifaany_rev", ///
   col(3) graphr(margin(r=1 l=0 t=1 b=0.5)) ysize(*1.4) xsize(*1.5) imargin(l=1 r=0 t=0 b=0) ///
   lmsize(*1.5) labsize(*0.8)
gr export "$figure/supplementary/efig7.png", replace width(5000)   
gr export "$figure/supplementary/efig7.pdf", replace as(pdf)
